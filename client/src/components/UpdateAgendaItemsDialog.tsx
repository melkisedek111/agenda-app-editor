...
const UpdateAgendaItemsDialog = () => {

    const { newAgenda, selectedAgendaItems, setSelectedAgendaItems, selectedAgendaItem, setSelectedAgendaItem } = useAgenda();
        ...
        try {
            ...
            for (const key in agendas) {
                ...
                if (agenda.isRequired) {
                    if (!agenda.value || agenda.value === 0) {
                        ...
                        if(findExistingOrder.length && selectedAgendaItem?.order != agenda.value) {
                            agenda.isError = true;
            ...
            if (!hasError.length) {
                if (selectedAgendaItem) {
                    const filteredNonSelectedItems = [...selectedAgendaItems].filter(item => item._id != selectedAgendaItem._id || item.order != selectedAgendaItem.order)
                    const sortedAgendaItems = [...filteredNonSelectedItems, values].sort((a, b) => a.order - b.order);
                    const totalCreditableDuration = [...sortedAgendaItems].reduce((acc, curr) => acc += curr.creditable ? Number(curr.duration) : 0, 0);
                    setSelectedAgendaItems(sortedAgendaItems);
                    toast({description: "Your selected agenda item has been updated."});
    ...
    useEffect(() => {
        const itemValues = JSON.parse(JSON.stringify(initialAgendaItemValues))
        if(selectedAgendaItem) {
            for(const key in itemValues) {
                if(key in selectedAgendaItem) {
                    itemValues[key as keyof TAgendaValues].value = selectedAgendaItem[key as keyof TAgendaValues];
                }
            }
            
            setAgendaValues(itemValues)
        }
    }, [selectedAgendaItem])

    return (
        <Dialog open={!!selectedAgendaItem} onOpenChange={() => setSelectedAgendaItem(undefined)}>
            <DialogContent className="sm:max-w-[640px]">
                <DialogHeader>
                    <DialogTitle>Update Agenda Item</DialogTitle>
                    <DialogDescription>
                        Please fill up the fields to update agenda item.
                    </DialogDescription>
                </DialogHeader>
                ...
                <DialogFooter>
                    <Button type="submit" onClick={handleAgendaValuesSubmit} >Update</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
...
export default UpdateAgendaItemsDialog
